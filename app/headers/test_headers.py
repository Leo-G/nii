...
#from app.headers.models import Headers
...
add_data = """{
  "data": {
    ...
    {"headers": "How to build CRUD app with Python, Flask, SQLAlchemy and MySQL. Som reand456989@#$%^%> <html/>"}
         ,
...
class TestHeaders(unittest.TestCase):
    ...
    def test_01_add(self):
        ...
        rv = self.app.post('/api/v1/headers.json',
                           data=add_data, content_type="application/json")
        assert rv.status_code == 201

    def test_02_read_update(self):
        request = self.app.get('/api/v1/headers.json')
        dict = json.loads(request.data.decode('utf-8'))
        id = dict['data'][0]['id']
        rv = self.app.patch('/api/v1/headers/{}.json'.format(id),
    ...
    def test_03_delete(self):
        ...
        dict = json.loads(request.data.decode('utf-8'))
        id = dict['data'][0]['id']
        rv = self.app.delete('/api/v1/headers/{}.json'.format(id))
